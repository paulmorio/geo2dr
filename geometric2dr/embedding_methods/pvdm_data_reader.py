...
class PVDMCorpus(Dataset):
    ...
    def scan_corpus(self, min_count):
        ...
        for fname in self.graph_fname_list:
            for l in open(fname).readlines():
                ss = l.split()[
                    : self.window_size + 1
                ]  # as context may have new subgraphs we havent seen
                for s in ss:
                    subgraphs[s] += 1
    ...
    def pre_load_corpus(self):
        ...
        for _ in tqdm(range(self._subgraphcount)):
            ...
            for i in range(len(subgraphs_on_line)):
                ...
                if target_subgraph in self._subgraph_to_id_map:
                    target_graph_ids.append(self._graph_name_to_id_map[target_graph])
                    target_subgraph_ids.append(
                        self._subgraph_to_id_map[target_subgraph]
                    )
                    temp_subgraph_contexts = []
                    for subgraph_context in subgraphs_on_line_copy:
                        if subgraph_context in self._subgraph_to_id_map:
                            temp_subgraph_contexts.append(
                                self._subgraph_to_id_map[subgraph_context]
                            )
            ...
            target_context_pairs = list(
                zip(target_graph_ids, target_subgraph_ids, subgraph_contexts_ids)
            )
            shuffle(target_context_pairs)
            target_graph_ids, target_subgraph_ids, subgraph_contexts_ids = list(
                zip(*target_context_pairs)
            )

            negatives_per_context = [
                self.getNegatives(x, 10) for x in target_subgraph_ids
            ]
            graph_targetSubgraph_subgraphContexts_negatives = [
                (graphTarget, subgraphTarget, subgraphContext, negatives)
                for (graphTarget, subgraphTarget, subgraphContext, negatives) in zip(
                    target_graph_ids,
                    target_subgraph_ids,
                    subgraph_contexts_ids,
                    negatives_per_context,
            ...
            self.context_pair_dataset.append(
                graph_targetSubgraph_subgraphContexts_negatives
            )
    ...
    def collate(batches):
        all_targets = [
            tgraph for batch in batches for tgraph, _, _, _ in batch if len(batch) > 0
        ]
        all_contexts = [
            tsubgraph
            for batch in batches
            for _, tsubgraph, _, _ in batch
            if len(batch) > 0
        ]
        all_subgraph_contexts = [
            csubgraph
            for batch in batches
            for _, _, csubgraph, _ in batch
            if len(batch) > 0
        ]
        all_neg_contexts = [
            neg_tsubgraph
            for batch in batches
            for _, _, _, neg_tsubgraph in batch
            if len(batch) > 0
        ]
        ...
        return (
            ...
            torch.LongTensor(all_subgraph_contexts),
...
if __name__ == "__main__":
    ...
    in_mem_pvdm_corpus = PVDMCorpus(
        corpus_dir, extension=".awe_4_nodes", max_files=60, window_size=3
    )
    pvdm_dataloader = DataLoader(
        in_mem_pvdm_corpus,
        batch_size=4,
        shuffle=False,
        num_workers=0,
        collate_fn=in_mem_pvdm_corpus.collate,
    )
