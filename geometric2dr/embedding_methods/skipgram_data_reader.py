...
class SkipgramCorpus(Dataset):
    ...
    def scan_and_load_corpus(self):
        ...
        self.graph_fname_list = sorted(
            get_files(self.corpus_dir, self.extension, self.max_files)
        )
    ...
    def __getitem__(self, idx):
        ...
        target_context_negatives = [
            ...
            for (target, context, negatives) in zip(
                target_subgraph_ids, context_subgraph_ids, negatives_per_context
            )
        ]
...
class InMemorySkipgramCorpus(Dataset):
    ...
    def preload_corpus(self):
        ...
        for _ in tqdm(range(self._subgraphcount)):
            ...
            line_id = self.subgraph_index
            target_subgraph = graph_contents[line_id].split()[
                0
            ]  # first item on the line
            subgraph_contexts = graph_contents[line_id].split()[
        ...
        for inner in self.context_pair_dataset:
            ...
            # print(x)
            if (
                len(temp_batch_target) == 128
                and len(temp_batch_context) == 128
                and len(temp_batch_neg) == 128
            ):
                self.context_pair_batches.append(
                    (
                        torch.LongTensor(temp_batch_target),
                        torch.LongTensor(temp_batch_context),
                        torch.LongTensor(temp_batch_neg),
                    )
                )
                temp_batch_target = []
                temp_batch_context = []
...
if __name__ == "__main__":
    ...
    mem_corpus = InMemorySkipgramCorpus(
        corpus_dir, extension=".spp", max_files=60, window_size=3
    )
