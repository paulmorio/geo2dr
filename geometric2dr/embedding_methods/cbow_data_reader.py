...
class CbowCorpus(Dataset):
	...
	def pre_load_corpus(self):
		...
		for _ in tqdm(range(self._subgraphcount)):
			...
			target_subgraph_ids, subgraph_contexts_ids = list(zip(*target_context_pairs))

			negatives_per_context = [self.getNegatives(x,10) for x in target_subgraph_ids]

			target_context_negatives = [(target, context, negatives) for (target, context, negatives) in zip(target_subgraph_ids, subgraph_contexts_ids, negatives_per_context)]
