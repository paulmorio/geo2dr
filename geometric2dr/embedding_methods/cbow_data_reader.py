...
class CbowCorpus(Dataset):
    ...
    def initTableNegatives(self):
        ...
        count = np.round(ratio * CbowCorpus.NEGATIVE_TABLE_SIZE)
    ...
    def pre_load_corpus(self):
        ...
        for _ in tqdm(range(self._subgraphcount)):
            ...
            target_context_pairs = list(zip(target_subgraph_ids, subgraph_contexts_ids))
            shuffle(target_context_pairs)
            target_subgraph_ids, subgraph_contexts_ids = list(
            ...
            target_context_negatives = [
                ...
                for (target, context, negatives) in zip(
                    target_subgraph_ids, subgraph_contexts_ids, negatives_per_context
                )
            ]
...
if __name__ == "__main__":
    ...
    corpus = CbowCorpus(corpus_dir, extension=".spp", max_files=60, window_size=3)
