    ...
    for i, node in enumerate(rw_graph):
        ...
        aw = [
            str(
                walk_ids[
                    anonymous_walk_node(
                        graph, rw_graph, node, awe_length, label_setting
                    )
                ]
            )
...
def awe_corpus(
    corpus_dir, awe_length, label_setting, neighborhood_size=10, saving_graph_docs=True
):
    ...
    for gexf_fh in tqdm(graph_files):
        ...
        aws_graph = anonymous_walks(
            graph, neighborhood_size, walk_ids, awe_length, label_setting
        )
    ...
    prob_map = {
        gidx: {
            graphlet: count / float(sum(anon_walk_patterns.values()))
            for graphlet, count in anon_walk_patterns.items()
        }
        for gidx, anon_walk_patterns in graph_map.items()
    }
...
if __name__ == "__main__":
    ...
    corpus, vocabulary, prob_map, num_graphs, graph_map = awe_corpus(
        corpus_dir, awe_length, label_setting, saving_graph_docs=True
    )
