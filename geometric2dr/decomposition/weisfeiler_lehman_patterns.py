...
def initial_relabel(graph, node_label_attr_name="Label"):
    ...
    for node in graph.nodes():
        graph.nodes[node]["relabel"] = {}  # make a dictionary attribute
    ...
    for node in graph.nodes():
        ...
        if not label in label_to_compressed_label_map:
            ...
            graph.nodes[node]["relabel"][0] = "0+" + str(
                label_to_compressed_label_map[label]
            )
...
def wl_relabel(graph, it):
    ...
    for node in graph.nodes():
        ...
        prev_iter_node_label = get_int_node_label(
            graph.nodes[node]["relabel"][prev_iter]
        )  # just a int ("1") in first it 0
        node_label = [prev_iter_node_label]
        neighbours = list(nx.all_neighbors(graph, node))
        neighbourhood_label = sorted(
            [
                get_int_node_label(graph.nodes[nei]["relabel"][prev_iter])
        ...
        if not node_neighbourhood_label in label_to_compressed_label_map:
            ...
            graph.nodes[node]["relabel"][it] = (
                str(it)
                + "+"
                + str(label_to_compressed_label_map[node_neighbourhood_label])
            )
    return graph
    ...
    with open(open_fname, "w") as fh:
        for n, d in graph.nodes(data=True):
            for it in range(0, max_h + 1):
                ...
                NeisLabelsSameDeg = list(
                    set(
                        [
                            graph.nodes[nei]["relabel"][it]
                            for nei in nx.all_neighbors(graph, n)
                        ]
                    )
                )  # neighbours  on iteration it basically
                # if it != max_h:
...
def wl_corpus(fnames, max_h, node_label_attr_name="Label"):
    ...
    compressed_labels_map_list = (
        []
    )  # list of compressed labels maps that can be used to go backwards
    ...
    for it in range(1, max_h + 1):
        ...
        print(
            "num of WL rooted subgraphs in iter {} is {}".format(
                it, len(label_to_compressed_label_map)
            )
        )
    ...
    for fname, g in zip(fnames, graphs):
        ...
        for n, d in g.nodes(data=True):
            for it in range(0, max_h + 1):
                ...
                NeisLabelsSameDeg = list(
                    set([g.nodes[nei]["relabel"][it] for nei in nx.all_neighbors(g, n)])
                )
